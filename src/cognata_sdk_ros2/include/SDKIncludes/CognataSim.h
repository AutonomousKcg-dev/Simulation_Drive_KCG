...
namespace SDK
{ 
/** \defgroup CognataSDK Cognata SDK API
 * Direct Api of the Cognata SDK
*/
/** \defgroup CarCommands Car Commands
 * Specific methods related to controlling vehicles
*/
/*!
 ...
 *  - RGBCamera
 *  - DepthCamera
 *  - SemanticClassLabeling
 *  - SemanticInstanceLabeling
 *  - SemanticMaterialLabeling
 *  - SemanticStructureLabeling
 *  - ThermalCamera
 *  - CameraGT
 *  - DynamicObjectGT
 *  - CarTelemetries
 ...
 *  - CarsInSimulationChangeCallback
 * 
*/
template<class T>
class Sensor
...
{
    public:
        SpawnObjectCreationInfo() = default;
...
using SemanticMaterialLabeling = Sensor<SemanticMaterialLabelingOutput>;

using SemanticStructureLabeling = Sensor<SemanticStructureLabelingOutput>;
...
/**
 * @brief Register a callback function to be called when the car list in the simulation is updated
 * 
 ...
 */
    ...
    */
    ...
    //! Allows cloning a sensor preset from one vehicle to another 
    /*!
     *
     * \code
     *
        ...
        private:
            std::unique_ptr<CognataSim> mSim = std::make_unique<CognataSim>();
        public:
            void cloneSensorPresetToCar(unsigned int sourceCarId, unsigned int destCarId)
            {
                CognataErr error;
                error = cloneSensorPreset(sourceCarId, destCarId);

                if (error != COGNATA_ERR_OK)
                {
                    cerr << "cloneSensorPreset Failed. Return code: " << CognataErr_Name(error) << endl;
                    return;
                }
            }
        };
    ...
    \param fromActorID - id of source vehicle the has a sensor preset associated with it
    \param toActorID - id of destination vehicle which will be associated with the cloned sensor preset
    ...
    \return COGNATA_ERR_CONNECTION_TIMEOUT on connection timeout
    \return COGNATA_ERR_UNEXPECTED on unexpected error
    \return COGNATA_ERR_BAD_PARAMS when incorrect vehicle ids are passed
    */
    CognataErr cloneSensorPreset(unsigned int fromActorID, unsigned int toActorID);
 ...
 */
    ...
    */
     ...
     * 
     * @param script Script holding the desired simulation trigger
     * @param speed Value to set initial driving speed to
     */
     ...
     * 
     * @param script Script holding the desired simulation trigger
     * @param desiredSpeed Value to set desired driving speed to
     */
     ...
     * 
     * @param script Script holding the desired simulation trigger
     * @param x Desired x coordinate value
     * @param y Desired y coordinate value
     */
...
protected:
    template<class T>
    CognataErr addRegisteredSensorCallback(unsigned int carID, unsigned int sensorID, Sensor<T>& sensor);
    ...
    void handleSensorReport(const EngineToClientMessage& msg);
    void handleSensorMsg(const EngineToClientMessage& msg, const SensorOutput& sensorOutput);
    void handleAbstractSensorMsg(const EngineToClientMessage& msg, const SensorOutput& sensorOutput);
    ...
    // sensor callback class, saves sensor callbacks associated with a sensorID
    // used in conjection with a vehicleID map
    class SensorCallbackHandler;    
    std::unordered_map<unsigned int, std::shared_ptr<SensorCallbackHandler>> m_vehicleCallbackMap;
...
// explicit instantiation decliration
extern template CognataErr CognataSim::registerSensor<CarTelemetriesOutput>(unsigned int, unsigned int, Sensor<CarTelemetriesOutput>&);
extern template CognataErr CognataSim::registerSensor<DepthCameraOutput>(unsigned int, unsigned int, Sensor<DepthCameraOutput>&);
extern template CognataErr CognataSim::registerSensor<GPSOutput>(unsigned int, unsigned int, Sensor<GPSOutput>&);
extern template CognataErr CognataSim::registerSensor<LaneDetectorOutput>(unsigned int, unsigned int, Sensor<LaneDetectorOutput>&);
extern template CognataErr CognataSim::registerSensor<LidarOutput>(unsigned int, unsigned int, Sensor<LidarOutput>&);
extern template CognataErr CognataSim::registerSensor<RadarOutput>(unsigned int, unsigned int, Sensor<RadarOutput>&);
extern template CognataErr CognataSim::registerSensor<RGBCameraOutput>(unsigned int, unsigned int, Sensor<RGBCameraOutput>&);
extern template CognataErr CognataSim::registerSensor<SemanticClassLabelingOutput>(unsigned int, unsigned int, Sensor<SemanticClassLabelingOutput>&);
extern template CognataErr CognataSim::registerSensor<SemanticInstanceLabelingOutput>(unsigned int, unsigned int, Sensor<SemanticInstanceLabelingOutput>&);
extern template CognataErr CognataSim::registerSensor<SemanticMaterialLabelingOutput>(unsigned int, unsigned int, Sensor<SemanticMaterialLabelingOutput>&);
extern template CognataErr CognataSim::registerSensor<SemanticStructureLabelingOutput>(unsigned int, unsigned int, Sensor<SemanticStructureLabelingOutput>&);
extern template CognataErr CognataSim::registerSensor<E2COutput>(unsigned int, unsigned int, Sensor<E2COutput>&);
extern template CognataErr CognataSim::registerSensor<ROIOutput>(unsigned int, unsigned int, Sensor<ROIOutput>&);
extern template CognataErr CognataSim::registerSensor<ThermalCameraOutput>(unsigned int, unsigned int, Sensor<ThermalCameraOutput>&);
extern template CognataErr CognataSim::registerSensor<DynamicObjectGroundTruthOutput>(unsigned int, unsigned int, Sensor<DynamicObjectGroundTruthOutput>&);
